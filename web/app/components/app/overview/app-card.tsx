...
function AppCard({
  ...
  const { data: appAccessSubjects } = useAppWhiteListSubjects(appDetail?.id, systemFeatures.webapp_auth.enabled && !!appDetail?.id)
  const effectiveAccessMode = appDetail?.access_mode ?? appAccessSubjects?.accessMode
  ...
  useEffect(() => {
    if (appDetail && appAccessSubjects) {
      if (effectiveAccessMode === AccessMode.SPECIFIC_GROUPS_MEMBERS && appAccessSubjects.groups?.length === 0 && appAccessSubjects.members?.length === 0)
